...
FILEBASE_RPC_URL=os.getenv('FILEBASE_RPC_URL','https://rpc.filebase.io').rstrip('/'); FILEBASE_RPC_TOKEN=os.getenv('FILEBASE_RPC_TOKEN','')
...
def fetch_json(uri):
    for u in ipfs_urls(uri):
        try:
            r=requests.get(u,timeout=18,headers={'Accept':'application/json,*/*','User-Agent':'VisualGenomeCloud/3.1'})
...
def health():
    ...
    try:
        c=conn(); dbok=bool(c); c and c.close()
    except Exception: pass
    return {'version':'3.1.0','ethereum':e.is_connected(),'base':b.is_connected(),'database':{'configured':bool(os.getenv('DATABASE_URL')),'ok':dbok},'filebase':{'configured':bool(FILEBASE_RPC_TOKEN)}}
...
class Draft(BaseModel): metadata:dict; live_metadata_uri:str=''; image_uri:str=''
