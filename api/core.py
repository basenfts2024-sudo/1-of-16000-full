...
UA={'User-Agent':'1of16000-cloud/5.2','Accept':'*/*'}
...
def ipfs_urls(uri):
    ...
    if uri.startswith('ipfs://'):
        ...
        return [f'https://dweb.link/ipfs/{t}',f'https://ipfs.io/ipfs/{t}',f'https://ipfs.filebase.io/ipfs/{t}',f'https://gateway.pinata.cloud/ipfs/{t}']
    if uri.startswith('/ipfs/'):return ipfs_urls('ipfs://'+uri[6:])
...
def normalize_image_uri(image, metadata_uri, resolved_metadata_url):
    ...
    if str(metadata_uri).startswith('ipfs://'):
        ...
        if '/' in base:return f'ipfs://{base.rsplit("/",1)[0]}/{image.lstrip("./")}'
    if resolved_metadata_url and resolved_metadata_url.startswith(('http://','https://')):return urljoin(resolved_metadata_url,image)
    return image
...
def resolve_image(image_uri):
    ...
    for u in candidates:
        try:
            ...
            ok=r.ok and ('image/' in ctype or 'svg' in ctype or not ctype)
            r.close()
            if ok:return u,candidates
        except Exception:pass
    return candidates[0],candidates

def filebase_add_bytes(data:bytes,filename:str,content_type:str):
    if not FILEBASE_RPC_TOKEN:raise HTTPException(503,'Filebase publishing is not configured yet')
    r=requests.post(f'{FILEBASE_RPC_URL}/api/v0/add',params=[('cid-version','1'),('pin','true')],headers={'Authorization':f'Bearer {FILEBASE_RPC_TOKEN}'},files={'file':(filename,io.BytesIO(data),content_type)},timeout=120)
    if not r.ok:raise HTTPException(502,f'IPFS upload failed: HTTP {r.status_code}')
    p=r.json();cid=p.get('Hash')
    if not cid:raise HTTPException(502,'IPFS upload did not return a CID')
    return {'cid':cid,'uri':f'ipfs://{cid}','gateway':f'https://dweb.link/ipfs/{cid}'}
...
def health():
    ...
    return {'version':'5.2.0','ethereum':e.is_connected(),'base':b.is_connected(),'database':{'configured':configured(),'ok':dbok},'filebase':{'configured':bool(FILEBASE_RPC_TOKEN)},'opepen_source':'https://opepen.art'}
...
def token(tid:int):
    ...
    try:
        c=w.eth.contract(address=MAIN,abi=ABI);uri=c.functions.tokenURI(tid).call();owner=c.functions.ownerOf(tid).call()
    except Exception as e:raise HTTPException(502,f'Could not read token {tid} from Ethereum: {e}')
    meta,res=fetch_json(uri);raw_image=meta.get('image') or meta.get('image_url') or meta.get('image_data') or '';image_uri=normalize_image_uri(raw_image,uri,res);preview,candidates=resolve_image(image_uri)
    return {'token_id':tid,'owner':owner,'metadata_uri':uri,'resolved_metadata_url':res,'metadata':meta,'image_uri':image_uri,'image_preview':preview,'image_candidates':candidates,'image_resolved':bool(preview),'contract':MAIN}
...
async def filebase_add(file:UploadFile=File(...)):
    data=await file.read();return filebase_add_bytes(data,file.filename or 'artwork',file.content_type or 'application/octet-stream')

@router.post('/api/filebase/metadata')
def filebase_metadata(body:MetadataDraft):
    if body.token_id<1 or body.token_id>16000:raise HTTPException(400,'Token out of range')
    raw=json.dumps(body.metadata,ensure_ascii=False,indent=2).encode('utf-8')
    return filebase_add_bytes(raw,f'{body.token_id}.json','application/json')
